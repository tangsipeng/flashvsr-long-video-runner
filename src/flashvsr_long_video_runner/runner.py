...
def render_chunk(reader, chunk: ChunkManifest, upstream_module, pipe, scale: float, fps: float) -> None:
    ...
    rendered_frames = upstream_module.tensor2video(video)[: chunk.render_length]
    trimmed = rendered_frames[chunk.trim_start : chunk.trim_end]
    ...
    upstream_module.save_video(trimmed, str(out_path), fps=fps, quality=5)
    ...
    with upstream_runtime_dir(manifest.upstream.infer_script, manifest.upstream.weights_dir):
        ...
        try:
            for chunk_index in pending_chunk_indices(manifest, resume=resume):
                ...
                render_chunk(reader, chunk, upstream_module, pipe, manifest.scale, manifest.video.fps_float)
